...
void difftest_step(uint32_t eip) {
  ...
  // WARN who is controlling the is_skip_dut
  if (is_skip_dut) {
    is_skip_dut = false;
    return;
  }
  ...
  ref_difftest_exec(1);
  ref_difftest_getregs(&ref_r); // WARN shit we should change eflags into it
  ...
  for (int i = 0; i < 8; ++i) {
    if(cpu.gpr[i]._32 != ref_r.gpr[i]._32){
      ...
      printf("ABORT %*s%*x%*x\n", 10, reg_name(i, 4), 20, reg_l(i), 20, ref_r.gpr[i]._32);
      boom = true;
    }
  }
  ...
  if(cpu.eip != ref_r.eip){
    nemu_state = NEMU_ABORT;
    printf("ABORT %*s%*x%*x\n", 10, "eip", 20, cpu.eip, 20, ref_r.eip);
    boom = true;
  }
