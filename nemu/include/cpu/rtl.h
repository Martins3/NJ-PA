...
// define almost all the code for rtl arith
// interpret_rtl_add_M2G
...
// this is only code we should use !
...
make_rtl_arith_logic(idiv_r)
  ...
  // use C to implement some rtl function
...
// WARN: I can not understand why there is rtl_mv.
...
static inline void rtl_not(rtlreg_t *dest, const rtlreg_t* src1) {
  // TODO();
  // WARN maybe we should use rtl language
  *dest = ~(*src1);
}
...
static inline void rtl_sext(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  ...
  if(width == 4){
    // printf("WARN if this happened, it means I can not understand i386 clearly !\n");
    *dest = *src1;
    return;
  }
  rtlreg_t f = *src1 & (1 << (width * 8 - 1));
  int32_t m = ((int32_t)1 << 31) >> ((4 - width) * 8 - 1);
  // printf("---------> %x %x", f, m);
...
// move most significant bit
static inline void rtl_msb(rtlreg_t* dest, const rtlreg_t* src1, int width) {
  *dest = *src1 & (1 << (width * 8 - 1));
  // TODO();
}

// TODO bit field is understand clearly
...
static inline void rtl_update_ZF(const rtlreg_t* result, int width) {
  // TODO rtl language is requeired
...
static inline void rtl_update_SF(const rtlreg_t* result, int width) {
  ...
  // TODO: describe this language with rtl
  unsigned at = (*result) >>  ((width << 3) - 1);
  rtl_set_SF(&at);
}
